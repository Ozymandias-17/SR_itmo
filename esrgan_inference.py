...
def validate():
    ...
    os.makedirs(args.output_dir + f'_{args.model}', exist_ok=True)
    ...
    with torch.no_grad():
        for idx, (lr_imgs, hr_imgs) in enumerate(tqdm(val_loader, desc="Validation")):
            ...
            if idx < args.save_num:
                ...
                save_path = os.path.join(args.output_dir + f'_{args.model}', f"result_sample_{idx:03d}.png")
    ...
    res_path = os.path.join(args.output_dir + f'_{args.model}', "metrics.json")
...
def inference():
    ...
    parser.add_argument('--output', type=str, default='./img_results/output.png', 
                        help='Path to save upscaled image')
    parser.add_argument('--mode', type=str, default='rgb', 
                        help='Mode: rgb, rgb_lpips, yuv')
    args = parser.parse_args()
    ...
    output_dir = os.path.dirname(args.output + f'_{args.model}')
    ...
    print(f"Result is saved in: {args.output + f'_{args.model}'}")
